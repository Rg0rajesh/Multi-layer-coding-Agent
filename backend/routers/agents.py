...
"""
HTTP surface for the Human supervisor agent (C3) plus governance/memory endpoints.
"""
...
async def list_agent_runs(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
    result = await db.execute(select(AgentRun).where(AgentRun.task_id == task_id).order_by(AgentRun.started_at))
    return result.scalars().all()

@router.get("/{task_id}/pending-plan", response_model=PendingPlanOut)
async def get_pending_plan(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
    plan = await human_agent.get_pending_plan_async(str(task_id))
    return PendingPlanOut(pending=plan is not None, plan=plan)

@router.post("/{task_id}/approve", status_code=status.HTTP_204_NO_CONTENT)
async def approve_plan(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
...
async def reject_plan(task_id: UUID, body: RejectRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
...
async def edit_plan(task_id: UUID, body: EditPlanRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
...
async def add_dynamic_role(task_id: UUID, body: AddRoleRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
    role_name = body.role_name.strip().upper()
    if role_name in RESERVED_AGENT_NAMES:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, f"'{role_name}' is a core pipeline agent — pick a different name for the new role")
    agent_run = AgentRun(task_id=task_id, agent_name=role_name, status="queued", input_data=body.config, stats={"requested_by": str(current_user.id), "reason": body.reason})
    db.add(agent_run)
    ...
    await emit_log(str(task_id), "HUMAN", "TASK", "✎", f"Dynamic role requested: {role_name}" + (f" — {body.reason}" if body.reason else ""))
    return agent_run

@router.get("/{task_id}/risk-score", response_model=RiskScoreOut)
async def get_risk_score(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
    row = (await db.execute(select(SessionRiskScore).where(SessionRiskScore.task_id == task_id))).scalar_one_or_none()
    if row is None:
...
async def get_grounding_report(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
    run = (await db.execute(select(AgentRun).where(AgentRun.task_id == task_id, AgentRun.agent_name == "GROUNDING").order_by(AgentRun.completed_at.desc().nullslast()).limit(1))).scalar_one_or_none()
...
async def get_identity_token(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    await _get_owned_task(db, task_id, current_user)
...
async def get_curated_memory(task_id: UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
